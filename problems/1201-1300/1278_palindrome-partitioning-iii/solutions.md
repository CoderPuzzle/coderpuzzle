...
![For s = "abc", k = 2, the cost table flags one mismatched pair per interval — s[0..2] pays 1 for the outer pair (a, c) — and the partition table fills to dp[2][3] = min(dp[1][1] + cost[1][2], dp[1][2] + cost[2][2]) = min(0 + 1, 1 + 0) = 1.](figures/solution-cost-table-partition-dp.svg)
