...
![With `cost = [1,1,1,2]` and `maxCost = 3`, the window grows over `abc` to a best sum of 3, and appending the trailing cost 2 (sum 5) forces two shrinks to a length-2 window that never beats it.](figures/solution-cost-window-slide.svg)
