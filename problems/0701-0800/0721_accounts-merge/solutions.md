...
![On Example 1's four accounts, each email is unioned with its account's first email so the two Johns join through the shared johnsmith@mail.com, merging into three output accounts with sorted emails.](figures/solution-union-find-emails.svg)
