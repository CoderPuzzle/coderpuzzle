...
![On `nums = [1,3,2]` vs `target = [2,1,4]` the padded difference sequence `0, -1, 2, -2, 0` has exactly two positive rises, `3` and `2`, whose sum `5` is the answer — each rise opens one interval operation that a later fall closes.](figures/solution-difference-rises.svg)
