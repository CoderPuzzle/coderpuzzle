...
![On code `<A>  <B> </A>   </B>`, the stack holds `[A]` then `[A, B]`, and when `</A>` arrives B is still on top, so the mismatch stops the scan and the document is invalid.](figures/solution-tag-stack-mismatch.svg)
