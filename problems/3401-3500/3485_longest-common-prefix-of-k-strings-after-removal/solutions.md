...
![For words = ["jump","run","run","jump","run"] with k = 2, removing one 'run' leaves top1 (the jump chain, cnt 2 ≥ k) off its path at depths 1–4, so the deepest witness at depth 4 gives answer 4.](figures/solution-trie-per-depth-top-two.svg)
