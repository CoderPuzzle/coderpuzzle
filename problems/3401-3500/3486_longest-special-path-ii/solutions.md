...
![On example 1's path 0→1→3→6→8 (prefix distances 0,1,2,7,10), each repeat of value 1 re-opens second behind top, and the one-duplicate window [second..depth] ends at node 8 with length 10 − 1 = 9; the tie 1→2→4 needs fewer nodes, giving [9,3].](figures/solution-two-sliding-windows.svg)
