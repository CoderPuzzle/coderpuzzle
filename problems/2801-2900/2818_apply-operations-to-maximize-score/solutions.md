...
![For Example 1's nums = [8, 3, 9, 3, 8] with k = 2 every prime score ties at 1, so left = [-1, 0, 1, 2, 3], right = [5, 5, 5, 5, 5] and ranges = [5, 4, 3, 2, 1]; the greedy sorts by value and spends both picks on the 9, whose range of 3 covers the budget, for a score of 81.](figures/solution-ranges-then-greedy.svg)
