...
![Over the example's four `addLand` calls the count steps 1, 1, 2, 3 — `addLand(0, 1)`
unions into `(0, 0)`'s set instead of raising the count, and each dashed ring marks a
set root.](figures/solution-union-find-count-steps.svg)
