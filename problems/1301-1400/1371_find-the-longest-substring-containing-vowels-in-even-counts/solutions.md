...
![Example 2's s = leetcodeisgreat with the running 5-bit mask under each index: equal masks pair up under arcs, and the widest equal-mask gap runs from the mask 0 before the string to the mask 0 at i = 4, spanning leetc, so the answer is 5.](figures/solution-vowel-parity-mask-arcs.svg)
