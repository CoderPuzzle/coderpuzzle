...
On nums = [2,1,4,3,5] with k = 10, each right endpoint shrinks its window
until sum × length drops below 10 and banks right − left + 1, giving
1 + 2 + 1 + 1 + 1 = 6.

![On nums = [2,1,4,3,5] with k = 10, each right endpoint shrinks its window until sum × length drops below 10 and banks right − left + 1, giving 1 + 2 + 1 + 1 + 1 = 6.](figures/solution-window-banked-counts.svg)
