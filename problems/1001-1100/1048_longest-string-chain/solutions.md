...
![On words = ["a","b","ba","bca","bda","bdca"], each word's one-deletion lookups that hit the map set its dp value, and the chain "a" -> "ba" -> "bda" -> "bdca" reaches dp = 4.](figures/solution-one-deletion-dp.svg)
