...
![For n = 3, each root 1, 2, 3 pairs its left-range shapes with its right-range shapes — a null against two two-node shapes, one node against one, two shapes against a null — summing to the five output trees.](figures/solution-build-range-pairs.svg)
