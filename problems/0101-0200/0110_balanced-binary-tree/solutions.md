...
![On the Example 2 tree [1,2,2,3,3,null,null,4,4] the bottom-up heights build to 3 and 1 at the root's two children, so the root stores the -1 sentinel when |3 - 1| = 2 breaks the balance rule, and height(root) = -1 answers false.](figures/solution-bottom-up-heights.svg)
